  ...
  <div class="grid grid-cols-12 lg:container mx-auto">
    <div class="col-span-12 h-[80dvh]">
      <div
        ...
        v-if="content"
        class="flex flex-col justify-center items-center w-full mt-8 xl:my-16 font-family-arial"
      >
        <h1
          class="text-4xl xl:text-7xl text-center font-bold bg-gradient-to-r from-[#780072] to-[#e00019] inline-block text-transparent bg-clip-text uppercase font-family-dancing"
        >
          {{ item.note }}
        </h1>
        <h1 class="text-xl text-center text-gray-600">
          {{ item.redirect }}
        </h1>
        <!-- | -->
        ...
        <!-- | -->
        <small class="text-center text-2xl mx-16 text-gray-600">
...
const { data: content } = await useAsyncData("contact", () => {
  return queryContent("/_partials/contact/index.ge")
    .where({ _partial: true })
    .find();
});
